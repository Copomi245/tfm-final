...
class BlueskyWorker {
    ...
    this.apiBaseUrl = 'http://192.168.1.140:3001'
    ...
    async loginToBluesky(page, username, password) {
        try {
            ...
            if (!loginButton) {
                console.log('ℹNo se encontró botón de login, posiblemente ya está logueado');
                return true;
            }
            ...
            (postElements) => {
                return postElements.map(post => {
                    try {
                        ...
                        // DETECCIÓN ESPECÍFICA PARA IMÁGENES DE CONTENIDO
                        let hasMediaContent = false;
                        
                        // 1. Buscar imágenes específicas de contenido (feed_thumbnail)
                        const contentImages = post.querySelectorAll('img[src*="feed_thumbnail"][style*="transition-duration: 0ms"]');

                        // 2. Buscar contenedores de media específicos
                        ...
                        // 4. Verificar que sean imágenes de contenido (no avatares)
    ...
    async scrapeBluesky(jobData, credentials, checkCancel, JobCancelledError) {
        const { palabras, fraseExacta, hashtags, usuarioBusqueda, fechaDesde, fechaHasta, tweetsLimit = 10 } = jobData;
        ...
        try {
           ...
           await checkForCancellation();
            ...
            const filePath = this.saveResults(posts, jobData);
...
async processJob(job) {
        ...
        try {
            console.log(`Procesando job ${job.id}:`, jobData);
            ...
            console.log(`Usando cuenta: ${credentials.username}`);
            ...
            const userId = trabajoExistente.user_id;
            ...
            const result = await this.scrapeBluesky(
                jobData, 
                credentials, 
                checkCancel, 
...
// Inicializar worker
console.log('Iniciando worker de Bluesky...');
try {
    const blueskyWorker = new BlueskyWorker();
    console.log('Worker de Bluesky listo. Esperando jobs...');
    console.log('-------------------------------------------');
} catch (error) {
